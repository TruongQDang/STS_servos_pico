...
bool STSServoDriver::setId(uint8_t const &old_servo_id, uint8_t const &new_servo_id)
{
...
int STSServoDriver::receiveMessage(
        uint8_t const &servo_id, 
...
bool STSServoDriver::writeRegisters(
        uint8_t const &servo_id, 
...
void STSServoDriver::convertIntToBytes(uint8_t const &servo_id, int const &value, uint8_t result[2])
{
...
{
        uint8_t const result = readRegister(servo_id, STSRegisters::MOVING_STATUS);
        return result > 0;
}
    ...
    bool const &asynchronous)
{
        ...
        return writeRegisters(servo_id, STSRegisters::TARGET_POSITION, sizeof(params), params, asynchronous);
}

bool STSServoDriver::setTargetVelocity(uint8_t const &servo_id, int const &velocity, bool const &asynchronous)
{
        return writeTwoBytesRegister(servo_id, STSRegisters::RUNNING_SPEED, velocity, asynchronous);
}

bool STSServoDriver::setMode(unsigned char const &servo_id, STSMode const &mode)
{

        return writeRegister(servo_id, STSRegisters::OPERATION_MODE, static_cast<unsigned char>(mode));
}
...
uint8_t STSServoDriver::readRegister(uint8_t const &servo_id, uint8_t const &register_id)
{
        uint8_t result = 0;
...
int16_t STSServoDriver::readTwoBytesRegister(uint8_t const &servo_id, uint8_t const &register_id)
{
